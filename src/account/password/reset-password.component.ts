...
export class ResetPasswordComponent extends AppComponentBase implements OnInit {
    ...
    ngOnInit(): void {
        let appUrl =  AppConsts.deepAppLink + '://reset-password-page?';
        ...
        if (this._activatedRoute.snapshot.queryParams['c']) {
            ...
            appUrl += 'userId=' + this.model.userId + '&';
            appUrl += 'resetCode=' + this.model.resetCode;
        ...
        if (AppConsts.deepAppLink) {
            this.saving = true;
            this.deeplinkService.deeplink(appUrl, null, true);
        }
    }
