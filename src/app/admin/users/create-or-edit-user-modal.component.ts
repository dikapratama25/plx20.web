...
import { CreateOrUpdateUserInput, OrganizationUnitDto, PasswordComplexitySetting, ProfileServiceProxy, BizInput, UserEditDto, UserRoleDto, UserServiceProxy, GetUserForEditOutput } from '@shared/service-proxies/service-proxies';
...
export class CreateOrEditUserModalComponent extends AppComponentBase {
    ...
    @ViewChild('companyCombobox', { static: true }) companyCombobox: ElementRef;
    @ViewChild('branchCombobox', { static: true }) branchCombobox: ElementRef;
    ...
    show(userId?: number): void {
        ...
        this.biz.bizRegId = this.appStorage.bizRegID;
        this.biz.bizLocId = this.appStorage.bizLocID;
    ...
    companyChanged(){
        ...
        if(this.biz.bizRegId!=""){

        
        this.spinnerService.show();
        let url = ProxyURL.GetBranchCombo+'bizregid='+encodeURIComponent(this.biz.bizRegId);
    ...
    loadCompanyCombo():void{
        this.spinnerService.show();
        let url = ProxyURL.GetCompanyCombo;
        this._proxy.request(url, RequestType.Get)
            .pipe(finalize(() => {
                this.spinnerService.hide();
            }))
